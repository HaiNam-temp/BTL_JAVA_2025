...
export class UpdateProductAdminComponent extends BaseComponent implements OnInit {  
  ...
  this.productService.getDetailProduct(this.productId).subscribe({
    next: (apiResponse: ApiResponse) => {
      ...
      // Kiểm tra nếu product_images tồn tại và là mảng
      if (Array.isArray(this.updatedProduct.product_images)) {
        this.updatedProduct.product_images.forEach((product_image: ProductImage) => {
          product_image.image_url = `${environment.apiBaseUrl}/products/images/${product_image.image_url}`;
        });
      } else {
        this.updatedProduct.product_images = []; // Đảm bảo không undefined
      }
    },
    complete: () => {},
    error: (error: HttpErrorResponse) => {
